...
if command -v xattr >/dev/null 2>&1; then
  xattr -dr com.apple.quarantine "$ROOT_DIR/★CodexReader.command" "$ROOT_DIR/★CodexReader Tunnel.command" "$ROOT_DIR/★ Codex Reader.app" "$ROOT_DIR/infra/macos/ensure-python-pdf-deps.sh" 2>/dev/null || true
fi

echo "Mac launch permissions fixed."
echo "Now double-click: ★CodexReader.command, ★CodexReader Tunnel.command, or ★ Codex Reader.app"
